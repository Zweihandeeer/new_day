...
class ChatProvider extends ChangeNotifier {
  ...
  // database instance
  Database? _database; // Propiedad para la base de datos
  ...
  Database? get database => _database; // Getter para la base de datos

  // Método para inicializar la base de datos
  Future<void> initializeDatabase() async {
    try {
      final databasePath = await getDatabasesPath();
      final path = "$databasePath/your_database.db";

      _database = await openDatabase(
        path,
        version: 1,
        onCreate: (db, version) async {
          // Crear la tabla de embeddings si no existe
          await db.execute('''
      ...
      rethrow; // Relanzar el error para manejarlo en otro lugar si es necesario
    }
  }
  ...
  Future<void> setModel({required bool isTextOnly}) async {
    if (isTextOnly) {
      ...
      _model = _textModel ?? GenerativeModel(
        model: 'gemini-1.5-pro', // Asegúrate de que este nombre coincida con el de GeminiService
        apiKey: getApiKey(),
      ...
      // Usar el modelo con visión
      _model = _visionModel ?? GenerativeModel(
        model: 'gemini-1.5-flash', // Asegúrate de que este nombre coincida con el de GeminiService
        apiKey: getApiKey(),
        generationConfig: GenerationConfig(
          temperature: 0.4,
          topK: 32,
